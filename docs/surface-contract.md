...
| Exports and local pack server | `docpull export` for JSONL, Sheets CSV/TSV, n8n JSON, Vercel AI JSON, CrewAI JSON, warehouse NDJSON, Parquet, and agent references; `docpull serve` | `export_pack`, `create_pack_app`, `load_pack` | `export_pack`, `serve_pack_status` | Adapted |
