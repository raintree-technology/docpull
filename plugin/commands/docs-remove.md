---
description: Remove a user-defined source alias from sources.yaml, optionally deleting its cached Markdown.
...
2. **`--keep-cache` flag: remove alias only.**
   Call `remove_source(name=<source>, delete_cache=false)`. The cached Markdown stays; `/docs-search <source>` will keep working until the user runs this without the flag.
