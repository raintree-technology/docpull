...
def _duplicate_count(page_count: int = PAGE_COUNT) -> int:
    """Return the exact duplicate count generated by the synthetic server."""
    dup_step = int(1 / DUPLICATE_FRACTION)
    return sum(1 for i in range(1, page_count) if i % dup_step == 0)
...
async def server(monkeypatch):
    ...
    try:
        ...
        if os.environ.get("CI") == "true":
            raise
        pytest.skip(f"localhost bind unavailable: {exc}")
...
async def test_10k_pages_end_to_end(server, tmp_path: Path) -> None:
    ...
    expected_duplicates = _duplicate_count()
    expected_unique = PAGE_COUNT - expected_duplicates
    discovery_secs = (
    ...
    assert duplicates_seen >= int(expected_duplicates * 0.9), report
